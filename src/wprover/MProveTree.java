...
public class MProveTree extends JTree implements ActionListener {
	...
	public MProveTree(final GExpert gx, final DPanel dd,
			final drawTextProcess dpp) {
		gxInstance = gx;
		dpane = dd;
		dp = dpp;
		...
		final TreeCellOpaqueRender treeRender = new TreeCellOpaqueRender();
		...
		addMouseListener(new MouseListener() {
			...
			public void mouseClicked(final MouseEvent e) {
				...
				final int n = MProveTree.this.getRowCount();
				final Rectangle r = MProveTree.this.getRowBounds(n - 1);
				if (e.getY() > (r.getY() + r.getHeight()))
					MProveTree.this.setSelectionRow(-1);
				else {
					final TreePath path = MProveTree.this.getPathForLocation(
							e.getX(), e.getY());
					if (path != null) {
...
class msymbol extends mobject {
	...
	public static void createAllIcons() {
		...
		for (final String element : cSprefix) {
			final ImageIcon icon = GExpert.createImageIcon("images/dtree/"
					+ element + ".gif");
			if (icon == null)
				CMisc.print("Can not find image : " + element);
			else
				vlist.add(icon);
		}
	}
...
class mdrobj extends mobject {
	...
	static void createAllIcons() {
		for (final String pString : pStrings) {
			...
			if (icon != null)
				...
				CMisc.print("Can not find object icon " + pString);
		}
	}
	...
	public boolean ck_parallelogram() {
		...
		final GEPoint p3 = objlist.get(2);
		final GEPoint p4 = objlist.get(3);
		return drawbase.check_para(p1, p2, p3, p4)
				&& drawbase.check_para(p1, p4, p2, p3);
	}
...
class mobject {
	...
	public static mobject load(final DataInputStream in, final drawProcess dp)
			throws IOException {
		...
		public void Load(final DataInputStream in, final drawProcess dp) throws IOException {
			type = in.readInt();
		}
...
class meqterm {
	...
	public static meqterm Load(final DataInputStream in, final drawProcess dp)
			throws IOException {
		final int t = in.readInt();
