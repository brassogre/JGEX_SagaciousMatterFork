...
public class drawProcess extends drawbase implements Printable, ActionListener {
	...
	public void SetProportionLineAction(final int prop) {
		SetCurrentAction(drawbase.LRATIO);
		proportion = prop;
	}
	...
	public void clearAll() {
		...
		if ((gxInstance != null) && gxInstance.hasAFrame()) {
			final AnimatePanel ac = gxInstance.getAnimateDialog();
			if (ac != null)
				ac.stopA();
		}
		clearConstruction();
	...
	public void popLeadingVariableDialog() {
		final LeadVariableDialog dlg = new LeadVariableDialog(gxInstance);
	...
	public boolean calculate_a_point(final GEPoint p, final boolean d) {
		...
		if (((m1 != null) && (m2 == null) && (PolyBasic.deg(m1) == 1))
				|| ((m1 == null) && (m2 != null) && (PolyBasic.deg(m2) == 1))) {
		...
		if ((m1 == null) && (m2 != null) && d) {
			...
			assert(cp.y1.value == cp.y1.value);
			if (v == 1)
				r = poly.calculate_online(m2, parameter, cp.x1.xindex,
						cp.y1.xindex);
			else if (v == 2)
				r = poly.calculate_oncr(m2, parameter, cp.x1.xindex,
						cp.y1.xindex);
			if (r != null) {
				cp.x1.value = r[0];
				cp.y1.value = r[1];
	...
	public void charsetAndAddPoly(final boolean bRecalculate) {
		...
		if (plist != null) {
			TPoly plist2 = plist;
			while (plist2 != null) {
				pblist = poly.ppush(PolyBasic.pcopy(plist2.getPoly()), pblist);
	...
	public void optimizePolynomial() {
		...
		final GEPoint p1 = pointlist.get(0);
		final GEPoint p2 = pointlist.get(1);
		final int zeron[] = GeoPoly.getZeron();
		...
		for (final constraint cs : constraintlist) {
			...
			if ((t == constraint.PONLINE) || (t == constraint.INTER_LC)) {
				...
				final GELine l1 = (GELine) cs.getelement(1); // XXX This threw an exception one time (when loading a faulty file) CastException [Was GEPoint]
	...
	public static void addZeron(final int x, final int[] zeron) {
		for (int i = 0; zeron[i] != x; i++) {
			if (zeron[i] == 0) {
				zeron[i] = x;
				return;
			}
	...
	public void SetCurrentAction(final int type) {
		...
		if (gxInstance != null) {
			final CStyleDialog dlg = gxInstance.getStyleDialog();
	...
	public void defineSpecificAngle() {
		if (paraCounter != 1) {
			...
			if (v.size() == 0) {
				...
				.showMessageDialog(
						gxInstance,
						GExpert.getLanguage(1027,
								"Angle specification must be done before drawing anything"),
								GExpert.getLanguage(302, "Warning"),
								JOptionPane.WARNING_MESSAGE);
				return;
			}
			final SpecificAngleDialog dlg = new SpecificAngleDialog(null, 1, v);
			dlg.setVisible(true);
			return;
		}
		final SpecificAngleDialog dlg = new SpecificAngleDialog(gxInstance, 0, null);
		dlg.setVisible(true);
		if (!dlg.isOkPressed())
			return;
	...
	public Object popSelect(final ArrayList<GraphicEntity> v, final int x, final int y) {
		if (v.size() == 1)
			viewElement(v.get(0));
		if (v.size() > 1) {
			final SelectDialog sd = gxInstance.getSelectDialog();
	...
	public void RightMenuPopup(final double x, final double y) {
		if (gxInstance != null) {
			...
			if (!selected.isEmpty()) {
				...
				final JPanel d = panel;
				if (selected.size() > 1) {
					final SelectDialog dlg = new SelectDialog(gxInstance, selected);
				...
				new RightClickPopMenu(ge, gxInstance).show(panel, (int) x, (int) y);
			}
		}
	}
	...
	public static void getSmartPV(final GEPoint p1, final GEPoint p2) {
		...
		if ((Math.abs(x2 - x1) < CMisc.PIXEPS)
				&& ((Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2)) > (4 * CMisc.PIXEPS * CMisc.PIXEPS)))
			p2.setXY(x1, y2);
	...
	public CProveText SelectProveText(final double x, final double y) {
		if (cpfield == null)
			return null;
	...
	public void DWButtonDown(double x, double y) {
		...
		case SELECT: {
			...
			if (t == null) {
				if (cpfield != null) {
					final CProveText ct1 = cpfield.mouseMove(x, y);
					if (ct1 == null) {
						bClear = true;
						final CProveText ct = cpfield.select(x, y, false);
						if (ct != null) {
		...
		case ANIMATION: {
			...
			final AnimatePanel af = gxInstance.getAnimateDialog();
			if (line != null) {
				clearSelection();
				animate = new AnimateC(p, line, Width, Height);
				...
				if (c != null) {
					clearSelection();
					animate = new AnimateC(p, c, Width, Height);
					af.setAttribute(animate);
					gxInstance.showAnimatePane();
					SetCurrentAction(MOVE);
				} else {
					...
					if (ct != null) {
						clearSelection();
						animate = new AnimateC(p, ct, Width, Height);
						af.setAttribute(animate);
						gxInstance.showAnimatePane();
						SetCurrentAction(MOVE);
					}
		...
		case SETEQANGLE3P: {
			...
			if (SelectList.size() == 2) {
				...
				final SpecificAngleDialog dlg = new SpecificAngleDialog(
						gxInstance, 2, alist);
		...
		case SANGLE: {
			...
			if (n == 0) {
				...
				final double k1 = constraint.get_sp_ag_value(STATUS);
					...
					if (!SelectList.isEmpty()) {
						...
						if (findPolygon(SelectList) != poly) {
							...
							if (CMisc.TransComfirmed) {
								...
								final TransformConfirmDialog dlg = new TransformConfirmDialog(
	...
	public void dialog_addText(final GEText tc, final int x, final int y) {
		final TextFrame tf = new TextFrame(gxInstance, x, y);
		tf.setText(tc);
	...
	public void clearFlashAndAdd(final ArrayList<JFlash> v) {
		clearFlash();
		flashlist.addAll(v);
	}

	public void clear_but_angle() {
		for (final Iterator<JFlash> iter = flashlist.iterator(); iter.hasNext();) {
			final JFlash ff = iter.next();
	...
	public void clearFlash() {
		for (final JFlash ff : flashlist)
			ff.stop();
		flashlist.clear();
	}

	public void addFlash(final JFlash f) {
		if (f == null)
			return;
	...
	public void addCgFlash(final JCgFlash f1, final JCgFlash f2, final JFlash f) {
		final int size = flashlist.size();
		int n = 0;
		for (final JFlash fx : flashlist)
		...
		for (; true; i++) {
			int j = 0;
			for (j = 0; j < size; j++) {
				final JFlash fx = flashlist.get(j);
				if (fx instanceof JCgFlash) {
					final JCgFlash fx1 = (JCgFlash) fx;
					if (i == fx1.getDNum())
						break;
				}
			}
	...
	public void addFlash2(final JFlash f) {
	...
	public void addFlash1(final JFlash f) {
	...
	public void addFlashx(final JFlash f) {
		if (f == null)
			return;

		if (f instanceof JAngleFlash) {
			final JAngleFlash tf = (JAngleFlash) f;
			int num = 0;

			final GEPoint pt = getCommonPoint(tf.p1, tf.p2, tf.p3, tf.p4);
			if (pt != null)
				for (final JFlash obj : flashlist)
					if (obj instanceof JAngleFlash) {
						final JAngleFlash ff = (JAngleFlash) obj;
	...
	public boolean isFlashFinished() {
		...
		if (n > 1)
			return false;
		final JFlash f = flashlist.get(0);
		return (f.isfinished());
	...
	public void drawCurrentAct(final Graphics2D g2) {
		...
		case D_3PCIRCLE: {
			...
			if (STATUS == 2) {
				...
				g2.setStroke(CMisc.DashedStroke);
				g2.setColor(Color.red);
				if ((Math.abs(x) < CMisc.MAX_DRAW_LEN)
						&& (Math.abs(y) < CMisc.MAX_DRAW_LEN)
	...
	public void addFlashPolygon(final GEPolygon p1, final GEPolygon p2,
		...
		if (n != p2.getPtn())
			return;
		final JPolygonFlash f = new JPolygonFlash(panel, p1, p2, ct, xc, yc,
	...
	public static void drawAuxLine(final int x, final int y, final double k,
		...
		g2.setColor(Color.red);
		g2.setStroke(CMisc.DashedStroke);
		final double max = CMisc.MAX_DRAW_LEN;
	...
	public GEPoint createIntersectionPoint(final GELine line1, final GELine line2) {
		...
		if (line1 != null && line2 != null && !line1.isCoincidentWith(line2)) {
			...
			if (check_para(line1, line2)) {
				JOptionPane.showMessageDialog(gxInstance, GExpert.getLanguage(1028,
						"The two lines you selected are parallel" + ", and thus don't intersect."),
						GExpert.getLanguage(1029, "No intersection"),
						JOptionPane.ERROR_MESSAGE);
				return null;
			}
	...
	public GEPoint MeetCCToDefineAPoint(final GECircle c1, final GECircle c2,
		...
		if (!check_cc_inter(c1, c2)) {
			JOptionPane.showMessageDialog(gxInstance, GExpert.getLanguage(1033,
	...
	public void openFromXMLDocument(Element thisElement) {
		...
		if (thisElement != null) {
			...
			docVersion = GExpert.safeParseInt(thisElement.getAttribute("version"), 0);
			CurrentAction = MOVE;
			GridX = GExpert.safeParseInt(thisElement.getAttribute("GridX"), GridX, 10, 160);
			GridY = GExpert.safeParseInt(thisElement.getAttribute("GridY"), GridY, 10, 160);
			DRAWGRID = GExpert.safeParseBoolean(thisElement.getAttribute("GridVisible"), false);
			SNAP = GExpert.safeParseBoolean(thisElement.getAttribute("SnapToGrid"), false);
				...
				for (int i = 0; i < nlist.getLength(); ++i) {
					...
					if (nn != null) {
						...
						if (sEntry.equalsIgnoreCase("points")) {
							pnameCounter = GExpert.safeParseInt(nn.getTextContent(), -1);
							continue;
						}
						if (sEntry.equalsIgnoreCase("lines")) {
							plineCounter = GExpert.safeParseInt(nn.getTextContent(), -1);
							continue;
						}
						if (sEntry.equalsIgnoreCase("circles")) {
							pcircleCounter = GExpert.safeParseInt(nn.getTextContent(), -1);
							continue;
						}
						if (sEntry.equalsIgnoreCase("proof")) {
							cpfield = new CProveField((Element)nn);
							continue;
						}
						...
						if (sEntry.equalsIgnoreCase("backup_parameters")) {
							...
							if (n2list != null) {
								...
								for (int i2 = 0; i2 < n2list.getLength(); ++i2) {
									Node nn2 = n2list.item(i2);
									if (nn2 != null && nn2 instanceof Element) {
										paraBackup[paramindex++] = GExpert.safeParseDouble(nn2.getTextContent(), 0.0d);
...
//											ge = geNew;
	...
	boolean write_ps(final String name, final int stype, final boolean ptf,
		...
		final String sversion = "%Created By: " + Version.getNameAndVersion()
				+ "\n";
	...
	public UndoStruct UndoAdded(final Object tip, final boolean gr,
		...
		if (CMisc.id_count == currentUndo.id)
			return null;
		...
		if (gxInstance != null) {
			if (m && gxInstance.hasManualInputBar()
				...
				final mproveInputPanel input = gxInstance
	...
	public boolean Undo_step() {
		...
		if ((gxInstance != null) && gxInstance.hasManualInputBar()
			...
			final mproveInputPanel input = gxInstance.getMannalInputToolBar();
			if (input != null)
				input.undoStep(Undo);
		}
	...
	public void setUndoStructForDisPlay(final UndoStruct u,
		...
		if (((u != null) && u.flash) || compulsory_flash) {
			...
			final JObjectFlash f = new JObjectFlash(panel);
			f.setAt(panel, v);
			addFlash(f);
		}
	}
	...
	public JObjectFlash getObjectFlash(final GraphicEntity cc) {
		final JObjectFlash f = new JObjectFlash(panel);
		f.addFlashObject(cc);
		return f;
	}
	...
	public void setCurrentInstance(final GExpert gx) {
		gxInstance = gx;
	}
	...
	public int add_sp_angle_value(final int v) {
		// int n = paraCounter;
		final param p1 = parameter[paraCounter - 1] = new param(paraCounter++,
				0);
	...
	public void onDBClick(final GraphicEntity c) {
		...
		case GraphicEntity.TEXT:
			...
			if (tx.getType() == GEText.VALUE_TEXT) {
				final TextValueEditor dlg = new TextValueEditor(gxInstance);
				gxInstance.centerDialog(dlg);
				dlg.setText(tx);
				dlg.setVisible(true);
				break;
