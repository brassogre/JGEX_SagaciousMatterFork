 ...
 */
public class FlashPolygon extends Flash implements ActionListener {
    private int c1, c2;
    private ArrayList<GEPoint> vlist, vlist1;
    private int FTYPE = 0;
    private int nd = 0;
    private int nn = 0;
    private double dx, dy, da, dx1, dy1, dx2, dy2;
    private int[] mx, my;
    private GEPolygon poly2;
    private boolean R_Center = false;
    private double xc, yc;

    public FlashPolygon(JPanel p, GEPolygon poly1, GEPolygon poly2, boolean oc, double xc, double yc, int c1, int c2, int tt) {  // A B C D ....A
        super(p);

        vlist = new ArrayList<GEPoint>();
        vlist.addAll(poly1.points);
        vlist1 = new ArrayList<GEPoint>();
        vlist1.addAll(poly2.points);
        this.c1 = c1;
        this.c2 = c2;
        R_Center = oc;
        this.xc = xc;
  ...
  //      poly1.setVisible(false);
    ...
    private void init() {
        ...
        int s = (int) (Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2)) / UtilityMiscellaneous.getMoveStep());
        p1 = vlist.get(0);
        p2 = vlist1.get(0);
        p3 = vlist.get(1);
        p4 = vlist1.get(1);
...
//        double r = CAngle.get4pAngle(p1.getx() + x, p1.gety() + y, dx2, dy2, p2.getx(), p2.gety());
        double r = GEAngle.get4pAngle(p1.getx(), p1.gety(), p3.getx(), p3.gety(), p2.getx(), p2.gety(), p4.getx(), p4.gety());
        ...
        if (FTYPE == 0) {
            ...
            if (index <= this.nn) {
                for (int i = 0; i < ln; i++) {
                    GEPoint p = vlist.get(i);
                    GEPoint p1 = vlist1.get(i);
                    mx[i] = (int) (p.getx() + (p1.getx() - p.getx()) * index / nn);
                    my[i] = (int) (p.gety() + (p1.gety() - p.gety()) * index / nn);
                }
            } else
                return false;
        }
    ...
    public static void drawColoredTriangle(int[] p1, int[] p2, Graphics2D g2) {
