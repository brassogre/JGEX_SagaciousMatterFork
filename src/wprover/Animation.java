...
public class Animation {
    ...
    Object onObj = null;
    int onType = 0; // 1:line 2:circle 3:trace   // 4: prove
    ...
    double gap = UtilityMiscellaneous.ANIMATE_GAP;
    ...
    String getAnimationString() {
        ...
        if (onType == 1) {
            GELine ln = (GELine) onObj;
            GEPoint pl[] = ln.getTwoPointsOfLine();
            if (pl == null) {
                return null;
            }
            ...
            GECircle c = (GECircle) onObj;
            GEPoint p = c.getSidePoint();
            if (c.o == null || p == null) {
                return null;
            }
    ...
    public boolean loadAnimationString(String s, DrawPanelExtended dp) {
        if (!s.startsWith("ANI")) {
            return false;
        }
        s = s.substring(4);
        ...
        GEPoint p1 = dp.findPoint(name);
        GEPoint p2 = dp.findPoint(sx.trim());
        GEPoint p3 = dp.findPoint(sy.trim());
        int n = Integer.parseInt(loc.trim());
        Object obj = null;
        if (s1.equals("LN")) {
            {
                obj = dp.findLineGivenTwoPoints(p2, p3);
                if (obj == null) {
                    GELine ln = new GELine(p2, p3);
                    dp.addLineToList(ln);
                    obj = ln;
                }
                onType = 1;
        ...
        } else if (s1.equals("CR")) {
            {
                ...
                if (obj == null) {
                    GECircle c1 = new GECircle(p2, p3);
    ...
    Animation(GEPoint p, Object obj, double width, double height) {
        pA = p;
        onObj = obj;
        this.width = width;
        this.height = height;
        ...
        if (obj instanceof GELine) {
            ...
            onType = 2;
        } else if (obj instanceof GETrace) {
            onType = 3;
        } else {
            UtilityMiscellaneous.print("Error,undifined on type ");
        }
        reCalculate();
    ...
    public void reCalculate() {
        if (onType == 2) {
            ...
            GELine line = (GELine) onObj;
            GEPoint[] pp = line.getTwoPointsOfLine();
            if (line.isVertical()) {
    ...
    public void Save(DataOutputStream out) throws IOException {
        ...
        GraphicEntity c = (GraphicEntity) onObj;
        if (c == null) {
            out.writeInt(-1);
        } else {
            out.writeInt(c.m_id);
