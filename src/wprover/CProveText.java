...
public class CProveText {
    ...
    private CProveField proofFieldNested;
    ...
    public CProveText(UndoStruct un) {
        ...
        if ((un.m_type == UndoStruct.T_COMBINED_NODE || un.m_type == UndoStruct.T_PROVE_NODE) && !un.childundolist.isEmpty()) {
            proofFieldNested = new CProveField(un.childundolist);
        }
        m_undo = un;
    }

    public CProveText(ArrayList<cond> vvv, cond co, int index, boolean gc) {
        m_co = co;
        ...
        if (co.getPCO() == null) {
            ...
            proofFieldNested = new CProveField(vv, false);
        } else if (n > 0) {
    ...
    public void loadFromXMLDocument(Element thisElement) {
    	if (thisElement.getTagName().equalsIgnoreCase("proof_text")) {
	    	...
	    	visible = GExpert.safeParseBoolean(thisElement.getAttribute("visible"), defaultVisible);
	    	isexpand = GExpert.safeParseBoolean(thisElement.getAttribute("expanded"), defaultExpanded);
	    	cmsg = GExpert.safeParseColor(thisElement.getAttribute("head_color"), defaultHeaderColor);
	    	cmsg = GExpert.safeParseColor(thisElement.getAttribute("message_color"), defaultMessageColor);
	    	x = GExpert.safeParseDouble(thisElement.getAttribute("x"), 0);
	    	y = GExpert.safeParseDouble(thisElement.getAttribute("y"), 0);
	    		...
	    		for (int i = 0; i < nl.getLength(); ++i) {
	    			...
	    			if (nn instanceof Element) {
	    				...
	    				if (ee.getTagName().equalsIgnoreCase("proof")) {
	    					proofFieldNested = new CProveField(ee);
	    					return;
	    				}
	    			}
	    		}
    ...
    public CProveText redo_invisible_head(drawProcess dp) {
        ...
        CProveText ct = proofFieldNested.redo_invisible_head(dp);
        if (ct == null)
            return this;
        else
            return ct;
    ...
    public void getFlashObjectList(ArrayList<GraphicEntity> v, drawProcess dp) {
...
//    }
    ...
    public CProveText next_prove_step(drawProcess dp, CProveText cpt, Boolean find) {
...
//                  dp.redo_step(m_undo);
    ...
    public void Load(DataInputStream in, drawProcess dp) throws IOException {
        ...
        if (cp) {
            proofFieldNested = new CProveField();
