...
public class DialogConclusion extends DialogBase implements ActionListener, ItemListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -1068102971276521714L;
    ...
    final public static int CONCLUSION_ERROR = -1;
    ...
    private JButton bok;
    private JComboBox<Integer> bx1, bx2;
    ...
    public void setCns(cons c) {
        ...
        if (c.type == gib.CO_COLL) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                GEPoint px = gxInstance.dp.findPoint(c.pss[i].toString());
                b.setSelectedItem(px);
            }
        }
    }

    public DialogConclusion(DrawPanelFrame gx, String title) {
    ...
    public DialogConclusion(DrawPanelFrame frame) {
    ...
    private void init() {
        ...
        ic1 = DrawPanelFrame.createImageIcon("images/ptree/hook.gif");
        ic2 = DrawPanelFrame.createImageIcon("images/ptree/cross.gif");
        ic3 = DrawPanelFrame.createImageIcon("images/ptree/question.gif");
        ...
        bt = new JComboBox<String>(ss) {
			 ...
			 * 
			 */
			private static final long serialVersionUID = -1186124836616111660L;
        ...
        bx1 = new JComboBox<Integer>(ls);
        bx2 = new JComboBox<Integer>(ls);
    ...
    public void setPointNames(ArrayList<String> v) {
        for (JComboBox<GEPoint> b : vlist) {
            b.removeAllItems();
            for (String s : v ) {
                GEPoint p = new GEPoint(s);
                b.addItem(p);
            }
        }
        ...
        // Pane2.setPoints(v); XXX Check to see whether we need to set the point names.
    }
...
//    }
    ...
    public int showDialog(String s) {
        ...
        resetAllItem();
        this.setLocation((this.getOwner().getX()), this.getOwner().getY() + this.getOwner().getHeight() / 3);
        this.pack();
...
/*    private String vs(int id1, int id2) {
    ...
    private GEPoint vspt(int id1, int id2) {
        if (id1 == 0) {
            return (GEPoint) vlist.get(id2).getSelectedItem();
        } else {
            return (GEPoint) vlist1.get(id2).getSelectedItem();
        }
    }
//  "Collinear",
//            "Parallel",
...
//            "Segment Equation"
    ...
    public boolean checkValid(int id) {
        switch (id) {
            ...
            case 6:
                return DrawPanelBase.check_eqangle(vspt(0, 0), vspt(0, 1), vspt(0, 2), vspt(0, 3), vspt(1, 0), vspt(1, 1), vspt(1, 2), vspt(1, 3));
            ...
            case 9:  //Equalateral Triangle
                return DrawPanelBase.check_eqdistance(vspt(0, 0), vspt(0, 1), vspt(0, 1), vspt(0, 2)) &&
                        DrawPanelBase.check_eqdistance(vspt(0, 0), vspt(0, 1), vspt(0, 0), vspt(0, 2));
            case 10:
                return DrawPanelBase.check_bisect(vspt(0, 0), vspt(0, 1), vspt(0, 2));
            case 11: //Tangent
                return DrawPanelBase.check_perp(vspt(0, 0), vspt(0, 1), vspt(0, 2), vspt(0, 1));

            case 12: { //Eq_product
                double d1 = DrawPanelBase.sdistance(vspt(0, 0), vspt(0, 1));
                double d2 = DrawPanelBase.sdistance(vspt(0, 2), vspt(0, 3));
                double d3 = DrawPanelBase.sdistance(vspt(1, 0), vspt(1, 1));
                double d4 = DrawPanelBase.sdistance(vspt(1, 2), vspt(1, 3));
                return Math.abs(d1 * d2 - d3 * d4) < UtilityMiscellaneous.ZERO;
            }
            case 13: { //Ratio
                int t1 = getRatioValue(1);
                int t2 = getRatioValue(2);
                return DrawPanelBase.check_eqdistance(vspt(0, 0), vspt(0, 1), vspt(1, 0), vspt(1, 1), t1, t2);
            }
    ...
    public cons getProve() {
        ...
        for (JComboBox<GEPoint> jb : vlist) {
            if (jb.isEnabled() && jb.getSelectedIndex() >= 0) {
                GEPoint pp = (GEPoint) jb.getSelectedItem();
                c.add_pt(pp, index++);
                sn += " " + pp.getname();
            }
        }
        for (JComboBox<GEPoint> b : vlist1) {
            if (b.isEnabled() && b.getSelectedIndex() >= 0) {
            	GEPoint pp = (GEPoint) b.getSelectedItem();
                c.add_pt(pp, index++);
                sn += " " + pp.getname();
            }
        }
    ...
    class condPane extends JPanel implements ActionListener {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -9069541976040740776L;
		ArrayList<JComboBox<GEPoint>> bx = new ArrayList<JComboBox<GEPoint>>();
        ...
        public condPane() {
            ...
            p.add(label);
            for (int i = 0; i < 3; ++i) {
	            JComboBox<GEPoint> jb = new JComboBox<GEPoint>();
	            bx.add(jb);
	            p.add(jb);
            }
            p.add(badd);
        ...
        public void setStatus(int s) {
        	JComboBox<GEPoint> jb2 = bx.get(2);
        	if (s == 0) {
        ...
        public void selectAPoint(GEPoint o) {
            boolean set = false;
            for (JComboBox<GEPoint> jb : bx) {
                GEPoint p = (GEPoint) jb.getSelectedItem();
                if (p == null && jb.isEnabled()) {
                    if (set == false) {
                        jb.setSelectedItem(o);
                        set = true;
                    } else
                        return;
                }
        ...
        public void getSelected(ArrayList<GEPoint> v) {
        	for (JComboBox<GEPoint> jb : bx) {
        		if (jb.isEnabled()) {
        			GEPoint p = (GEPoint) jb.getSelectedItem();
        			assert(p != null);
