 ...
 */
public class PanelMProofInput extends JToolBar implements ActionListener {
    /**
	 * 
	 */
	private static final long serialVersionUID = -952403707216342785L;
    ...
    private PanelConclusion assertPane;
    ...
    public PanelMProofInput(DrawPanelFrame gx, DrawPanelOverlay pp, DrawPanelExtended dp, JTree tree) {
        ...
        ocPanel.add(button);
        button = new JToggleButton(DrawPanelFrame.createImageIcon("images/ptree/full_step.gif"));
        ...
        button.setUI(ui);
        ImageIcon upicon = DrawPanelFrame.createImageIcon("images/dtree/up1.gif");
        ocPanel.add(Box.createHorizontalStrut(2));
        ocPanel.add(button);
        ...
        assertPane = new PanelConclusion(gxInstance, this);
    ...
    public void buttonOK() {
        if (type == mobject.TEXT) {
        	...
        	mobject obj1 = (mobject) prefixPane.getUserObject();
            if (obj1 != obj)
                node.replace(obj, obj1);
        } else if (type == mobject.SYMBOL) {
        	mobject obj1 = (mobject)  symbolPane.getUserObject();
            if (obj1 != obj)
                node.replace(obj, obj1);
        } else if (type == mobject.ASSERT) {
        	mobject obj1 = assertPane.getUserObject();
            if (obj1 != obj)
                node.replace(obj, obj1);
        } else if (type == mobject.DRAW) {
        	mobject obj1 = (mobject) drawPane.getUserObject();
            if (obj1 != obj)
                node.replace(obj, obj1);
            tree.expandSelectedNode();
        } else if (type == mobject.DOBJECT) {
        	mobject obj1 = (mobject) objectPane.getUserObject();
            if (obj1 != obj)
                node.replace(obj, obj1);
        } else if (type == mobject.EQUATION) {
        	mobject obj1 = epane.getUserObject();
            if (obj1 != obj)
                node.replace(obj, obj1);
        } else if (type == mobject.RULE) {
        	mobject obj1 = rulepane.getUserObject();
            if (obj1 != obj)
                node.replace(obj, obj1);
        }
    ...
    class textPanel extends JPanel {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 7872486049065026270L;
		JTextField tpane;
        mtext tx;
        ...
        public textPanel() {
            ...
            tpane = new JTextField() {
				 ...
				 * 
				 */
				private static final long serialVersionUID = 4915218197624420193L;
    ...
    class prefixPanel extends JPanel {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 7646904900197730087L;
		JComboBox<String> box;
        mprefix pfix = null;

        public prefixPanel() {
            this.setLayout(new BoxLayout(this, BoxLayout.X_AXIS));
            box = new JComboBox<String>(mprefix.cSprefix);
    ...
    class symbolPanel extends JPanel implements ItemListener {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 7686833085219108569L;
    ...
    class drawPanel extends JPanel {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 6498838971074699378L;
		private mdraw draw;
        ...
        public drawPanel() {
            ...
            field = new JTextPane() {
				 ...
				 * 
				 */
				private static final long serialVersionUID = -1290607001787605403L;
    ...
    class dobjPanel extends JPanel implements ActionListener, ItemListener {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 2434669671417622463L;
		private JComboBox<GEPoint> box;
		//private int pnum;
        private ArrayList<JComboBox<GEPoint>> vlist = new ArrayList<JComboBox<GEPoint>>();
        private mdrobj dobj = null;
...
//            pane.add(Box.createHorizontalGlue());
        ...
        public Object getUserObject() {
            ...
            for (int i = 0; i < vlist.size(); i++) {
                JComboBox<GEPoint> b = vlist.get(i);
                if (b.isEnabled()) {
                    dobj.add((GEPoint)b.getSelectedItem());
        ...
        public void setUserObject(mdrobj d) {
            ...
            dobj = d;
            if (d != null) {
                dobj.fillJComboBox(box);

                int n = dobj.getObjectNum();
                for (int i = 0; i < vlist.size(); i++) {
                    JComboBox<GEPoint> bx = vlist.get(i);
                    if (i < n)
...
//                labelx.setText(d.getTip());
        ...
        public void reset() {
            for (int i = 0; i < vlist.size(); i++) {
                vlist.get(i).setSelectedIndex(-1);
            }
            dobj = null;
            box.setSelectedIndex(0);
            labelx.setText("");
    ...
    class rulePanel extends JPanel {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -6883656436833068598L;
		private JComboBox<String> box;
        private mrule rule;

        public rulePanel() {
            box = new JComboBox<String>(mrule.cStrings);
            this.add(box);
        }
    ...
    class CommonEquationPanel extends JPanel implements ActionListener {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -1472602996723764840L;
		private ArrayList<termPane> vlist = new ArrayList<termPane>();
        ...
        public JComboBox<String> getSymbolBox() {
            JComboBox<String> B = new JComboBox<String>(meqterm.cStrings);
        ...
        public void setUserObject(mequation eq) {
            ...
            if (eq != null)
                for (int i = 0; i < eq.getTermCount(); i++) {
                    termPane tm = this.getATermPane(i);
                    tm.setUserObject(eq.getTerm(i));
                }
            else
                this.getATermPane(0);
        }
        ...
        class termPane extends JPanel {
			 ...
			 * 
			 */
			private static final long serialVersionUID = 2976101362479165800L;
    ...
    class selectLabel extends JLabel implements MouseListener, MouseMotionListener {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -3016213595184465549L;
    ...
    class popSelectMenu1 extends JPopupMenu implements ActionListener {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -5688279157160440325L;
		private int type;
...
//            JPanel panel1 = new JPanel(new GridLayout(10, 3));
            ...
            for (int i = 0; i < 12; i++) {
                ...
                if (s.equalsIgnoreCase("Eqangle")) {
                    item = new popSelectMenuItem("", DrawPanelFrame.createImageIcon("images/symbol/sym_eqangle.gif"), 3, i);
                } else if (s.equalsIgnoreCase("Congruent")) {
                    item = new popSelectMenuItem("", DrawPanelFrame.createImageIcon("images/symbol/sym_congruent.gif"), 3, i);
                } else
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (type == 0)  // add
            {
                ...
                PanelMProofInput.this.setNodeValue(d1, obj);
            } else if (type == 1) {
                mobject obj = mobject.createObject(id1, id2);
    ...
    class popSelectMenuItem extends JMenuItem {
		 ...
		 * 
		 */
		private static final long serialVersionUID = -334032619600042227L;
		int m_id1, m_id2;
