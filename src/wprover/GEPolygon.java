...
public class GEPolygon extends GraphicEntity implements Pointed {
	...
	/**
	 * Sets the <code>points</code> to contain every <code>GEPoint</code> in <code>v</code>.
	 * @param v The <code>ArrayList<GraphicEntity></code> containing the 
	 * <code>GEPoint</code>s to serve as the vertices for this <code>GEPolygon</code>.
	 */
	...
	/**
	 * If the list of points contains only p, adds a second copy of p to the list and returns true
	 * Otherwise, adds p if and only if p is not already in the list.
	 * @param p The <code>GEPoint</code> to be added to the list of points.
	 * @return boolean signifying whether the method applied the exception where it contains two entries of the same point.
	 */
	...
	/** 
	 * Returns the {@code GEPoint} immediately before <code>p</code>.
	 * @param p The {@code GEPoint} whose predecessor one is seeking. 
	 * @return The {@code GEPoint} immediately before <code>p</code> in the list <code>points</code>.
	 * Returns {@code null} if <code>p</code> is not in <code>points</code> or is the first element.
	 */
	public GEPoint getPreviousPoint(GEPoint p) {
	...
	/** 
	 * Returns the {@code GEPoint} immediately after <code>p</code>.
	 * @param p The {@code GEPoint} whose follower one is seeking. 
	 * @return The {@code GEPoint} immediately after <code>p</code> in the list <code>points</code>.
	 * Returns {@code null} if <code>p</code> is not in <code>points</code> or is the last element.
	 */
