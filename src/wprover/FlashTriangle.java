 ...
 */
public class FlashTriangle extends Flash implements ActionListener {
    ...
    int[] dcx = new int[MAX];
    int[] dcy = new int[MAX];
    protected GEPoint A, B, C, A1, B1, C1;
    int fn = UtilityMiscellaneous.POLYGON_ANIMATION_LAG * 6 - 1;

    int ndt, fnt = 0;


    public FlashTriangle(JPanel p, GEPoint a, GEPoint b, GEPoint c, GEPoint a1,
                     GEPoint b1, GEPoint c1, boolean dir, int color) {
        super(p);
        this.color = color;
...
//            tn = 0;
    ...
    public boolean draw(Graphics2D g2) {
        ...
        if (nd == n) {
            int nc = UtilityMiscellaneous.POLYGON_ANIMATION_LAG * 2;
            int x = fn / nc;
            if (nc != 0 && x != 3) {
