 ...
 */
public class FlashSegmentMoving extends Flash implements ActionListener {
    ...
    public FlashSegmentMoving(JPanel p, GEPoint p1, GEPoint p2, GEPoint p3, GEPoint p4, int c1, int c2) {
        super(p);
    ...
    private void init() {
        double r = UtilityMiscellaneous.getMoveStep();
        ...
        double r2 = Math.PI + GEAngle.get3pAngle(xt2, yt2, p3.getx(), p3.gety(), p4.getx(), p4.gety());
        if (r2 >= Math.PI)
            r2 = r2 - 2 * Math.PI;
        if (r2 <= -Math.PI)
            r2 = r2 + 2 * Math.PI;
        ...
        if (p1 == p3 && p2 == p4 || p1 == p4 && p2 == p3)
            r2 = 0;
        else if (Math.abs(r2) < UtilityMiscellaneous.ZERO && rt)
            r2 = Math.PI;
