...
public class GExpert extends JFrame implements ActionListener, KeyListener,
	...
	private SelectDialog sdialog = null;
	private UndoEditDialog udialog = null;
	private CDialogProve pdialog = null;
	private concDialog cdialog = null;
	private RuleDialog rdialog = null;
	private NumCheckDialog ndialog = null;
	...
	public void init() {
		...
		setLocal();
		setLookAndFeel();
		setIconImage(GExpert.createImageIcon("images/gexicon.gif").getImage());
		//loadRules();
		loadPreference();
		loadLanguage();
		initAttribute();
	...
	void handlePreferences() {
		if (prefdialog == null)
			prefdialog = new MiscDialog(this);
	...
	public static void loadLanguage() {
		...
		final File f = new File(user_directory + "/language/" + CMisc.lan
				+ ".lan");
		language.load(f);
	...
	public RuleDialog getRulesDialog(final int n) {
//		if (rdialog == null) {
...
//		}
	...
	public SelectDialog getSelectDialog() {
		if (sdialog == null)
			sdialog = new SelectDialog(this, new ArrayList<GraphicEntity>());
		return sdialog;
	}
	...
	public concDialog getConclusionDialog() {
		if (cdialog == null) {
	...
	public JFileChooser getFileChooser() {
		if (filechooser == null) {
			...
			filechooser.setFileFilter(new JFileFilter("gex.xml"));
		}
	...
	public void createTipLabel() {
		...
		final Font f = CMisc.button_label_font;
		label2.setFont(f);
		label.setFont(f);
		...
		show_button = new TStateButton(
				GExpert.createImageIcon("images/ticon/show.gif"),
		...
		if (f.isDirectory()) {
			...
			for (final File content : contents)
				if (!content.isDirectory())
					iconPool.add(GExpert.createImageIcon(content.getPath()));
				else
	...
	private JRadioButtonMenuItem addRadioButtonMenuItem(final JMenu bar,
		...
		if ((sName != null) && !sName.isEmpty()) {
			...
			menuitem.setText(GExpert.getLanguage(sName));
		}
	...
	public void sendAction(final String command, final Object src) {
		 ...
		 */else if (command.equals("Open"))
			...
			dp.SetCurrentAction(drawProcess.VIEWELEMENT);
			...
			_command = command;
			final String sx1 = GExpert.getLanguage(command);
			...
			if (command.equalsIgnoreCase("select"))
				...
				dp.SetCurrentAction(drawbase.PERPWITHFOOT);
			else if (command.equalsIgnoreCase("angle"))
				dp.SetCurrentAction(drawbase.D_ANGLE);
				...
				dp.SetCurrentAction(drawbase.SETEQANGLE);
				...
				if (ps.equalsIgnoreCase("Other..")) {
					...
					dp.SetCurrentAction(drawbase.LRATIO);
					final int[] t = GExpert.parse2Int(s);
					dp.setParameter(t[0], t[1]);
					setTipText(s);
				}
				...
				try {
					...
					ps = language.getEnglish(ps);
					if (ps.equalsIgnoreCase("Other..")) {
						String s = JOptionPane.showInputDialog(this, GExpert
								.getLanguage(1053,
				...
				final TextValueEditor dlg = new TextValueEditor(this);
				centerDialog(dlg);
				dlg.setVisible(true);
			}
	...
	public void saveProofAsGIF() {
		...
		final RectChooser1 r1 = new RectChooser1(this);
		centerDialog(r1);
		r1.setVisible(true);
		if (!r1.getResult())
			return;
		...
		try {
			final DataOutputStream out = drawProcess.openOutputFile(ff
					.getPath());
	...
	public void saveAsGIF() {
		...
		final GIFOptionDialog dlg = new GIFOptionDialog(this, getLanguage(2303,
				"GIF Option"));
		centerDialog(dlg);
		...
		Rectangle rect = null;
		final RectChooser rchoose = new RectChooser(this);
		...
		final GifProcessDialog dlg1 = new GifProcessDialog(getFrame());
		centerDialog(dlg1);
		dlg1.setTotal(n);
	...
	public void saveAsImage() {
		...
		if (s.length > 0) {
			...
			for (final String element : s) {
				final JFileFilter f = new JFileFilter(element);
	...
	protected void addRightButtons(final JToolBar toolBar) {
		...
		anButton.setToolTipText(GExpert.getLanguageTip("animate_start"));
		toolBar.add(button);
	...
	protected static ImageIcon createImageIcon(final String path) {
		final java.net.URL imgURL = GExpert.class.getResource(path);
		if (imgURL == null)
			return null;
	...
	public static void setLookAndFeel() {
		...
		try {
			...
			final String f = CMisc.lookAndFeel;
