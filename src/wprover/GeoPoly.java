...
public class GeoPoly extends PolyBasic {
	...
	private static Set<Integer> z = new HashSet<Integer>();

	private GeoPoly() {
	}

	public static boolean addZ(int n) {
		boolean bAlreadyPresentFlag = false;
		if (n > 0) {
			bAlreadyPresentFlag = z.add(n);
		}
		return bAlreadyPresentFlag;
	}

	public static boolean checkZ(int n) {
		return z.contains(n);
	}
	...
	static TMono ppdd(int x, int y) //new and add
	{
		if (UtilityMiscellaneous.POINT_TRANS) {
			// This half of the method examines whether x and y have already been placed in the z set.
			// It is done if the program is set to optimize its polynomials
			if (checkZ(x)) {
				if (checkZ(y))
					return null; // (pzero());
				else
	...
	////////////////////////////////////////////////////
	public static TMono mm_poly(cndg c, DrawPanel dp) {
...
//	public static void ndg_gbasis(final ArrayList<TMono> listInput1, final ArrayList<TMono> listInput2, ArrayList<TMono> listOutput) {
//		if (listInput1 != null && !listInput1.isEmpty()) {
//
//			int n = listInput1.size() + 2;
//			int d = -1;
//			for (TMono m : listInput1) {
//				m = n_ndg(m, d);
//				listOutput.add(m);
//				--d;
//			}
//
//			upValueTM(listOutput, n);
//			upValueTM(listInput2, n);
//			g_basis(listOutput);
//
//			upValueTM(listOutput, -n);
//			upValueTM(listInput2, -n);
