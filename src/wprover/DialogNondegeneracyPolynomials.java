...
public class DialogNondegeneracyPolynomials extends DialogBase implements ActionListener, MouseMotionListener, MouseListener, ChangeListener {
    /**
	 * 
	 */
    ...
    public DialogNondegeneracyPolynomials(DrawPanelFrame gx, gterm gt, DrawPanel dp) {
        ...
        getContentPane().add(new JScrollPane(panelx));
        setSize(WD, HD);
        int w = gx.getWidth();
        int h = gx.getHeight();
        int x = gx.getX();
        int y = gx.getY();
        setLocation(x + w / 2 - WD / 2, y + h / 2 - HD / 2);
    ...
    public void stateChanged(ChangeEvent e) {
        DialogNondegeneracyPolynomials.this.setTitle(tt.getTitleAt(tt.getSelectedIndex()));
    }
