...
public class Rules {
	...
	// XXX Need to load these in from a resource so that they can have unicode characters and be more easily read.
    ...
    final public static String[] GDD = {
            ...
            "* 6",
            "In ?ACB, with ?C = [1], if midpoint(E,A B), then Circumcenter (E,A B C) and EA = EB = EC.",

            "* 7",
            "If AB ? CD and CD ? EF, then AB ? EF.",

            "* 8",
            "If AC ? BC and AD ? BD, then Cyclic(A,B,C,D).",

            "* 9",
            "For a circle c(O,AB) and a point C on Circle c,if Collinear(A,B,O), then AC ? BC and ?ACB = [1].",

            "* 10",
            "If AB is the diameter of a circle with C on the circle, then AC ? BC",
    ...
    final public static String[] FULL =
            {
                    ...
                    "* 9",
                    " If Cyclic(A,B,M,N), then ?[AB,AM] = ?[BN,NM]. Hence, ?[AB,CD] = ?[AB,AM] + ?[AM,CD] = ?[BN,NM] + ?[AM,CD].",
                    ...
                    "* 14",
                    " For any circle(O,ABCD), if O is the intersection of AD and BC, then ?[AOB] = 2??[CDA].",

                    "* 15",
                    " For any circle(O,AB),we have ?[OAB] = ?[ABO]. ",

                    "* 16",
                    " If triangle ABE is a right triangle and O is the midpoint of the hypotenuse AB, then for any line CD, we have",
                    " ?[AB,CD] =  ?[BAE] + ?[1] + ?[BE,CD].",

                    "* 17",
                    " For any circle(O,ABC) with D as midpoint of BC, we have ?[AB,EF] = ?[BOD] + ?[AC,EF].",

                    "* 18",
                    " For any circle(O,ABF) with E as the midpoint of AF, we have ?[AB,CD] = ?[EOA] + ?[BF,CD].",

                    "* 19",
                    " If G is the orthocenter of A,B,C, then ?[AE,HI] = ?[AE,BE] + ?[BE,HI] = ?[1] + ?[BE,HI]",

                    "* 20",
                    "If A is the incenter of P2,K,I, then ?[AB,CD] = ?[AB,BK] + ?[BK,CD] = ?[1] + ?[KI,KA] + ?[IA,IK] + ?[BK,CD].",
                    ...
                    "*  29",
                    " If AB = AC = BC, (i.e., ABC is an equilateral triangle), then 3??[AB,BC] = ?[0].  ",
