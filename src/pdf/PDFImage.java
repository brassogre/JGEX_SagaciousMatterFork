...
public class PDFImage extends PDFStream implements ImageObserver, Serializable
{
	 ...
	 * @return a <code>String</code> value
	 */
	private static String base85Encoding(String stringToEncode)
		...
		ArrayList<Integer> digitVector = new ArrayList<Integer>();
		...
		for ( int i = 0; i < digitVector.size(); i++) {
			char c = (char) (digitVector.get(i).intValue() + 33);
			sb.append(c);
		}
    ...
    os.write(">\nendstream\nendobj\n\n".getBytes());
		...
		try (DeflaterOutputStream dos = new DeflaterOutputStream(b)) {
			buf.writeTo(dos);
			dos.finish();
			dos.close();
		}
	...
	public void write(OutputStream os) throws IOException
	{
		...
		ByteArrayOutputStream bos1 = getStream();
