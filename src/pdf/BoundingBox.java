...
public class BoundingBox extends Rectangle
{
  /**
	 * 
	 */
	private static final long serialVersionUID = 4898875913435813952L;
...
/** Percent f line height to space lines */
   ...
   */
  private static int getWrappedHeight(String[] strings, FontMetrics fm, int padding) {
   ...
   * @param
   */
  private static String[] createStringArray(String      string, 
                                     FontMetrics fm, 
                                     int         padding,
                                     int         pw) {
    if (string == null) {
    ...
    while(st.hasMoreTokens()) {
      ...
      if ((fm.stringWidth(tempBuffer.toString()) < lw) && 
       ...
       continue;
        ...
        returnVector.add(finalBuffer.toString());
    ...
    for (int i = 0; i < len; i++) {
      String curStr = returnVector.get(i);
   ...
   * @return a <code>boolean</code> value
   */
  private static boolean checkHAlign(int hAlign) {
   ...
   * @return a <code>boolean</code> value
   */
  private static boolean checkVAlign(int vAlign) {
