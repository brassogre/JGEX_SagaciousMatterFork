...
public class BigFraction implements Cloneable, Comparable<Object>, Serializable {
    /**
	 * 
	 */
	private static final long serialVersionUID = -1669720306445750273L;
	...
	private static boolean gt(BigInteger x, BigInteger y) {
        return x.compareTo(y) > 0;
    }

    private static boolean gteq(BigInteger x, BigInteger y) {
        return x.compareTo(y) >= 0;
    }
    ...
    private static boolean lt(BigInteger x, BigInteger y) {
        return x.compareTo(y) < 0;
    }

    @SuppressWarnings("unused")
	private static boolean lteq(BigInteger x, BigInteger y) {
        return x.compareTo(y) <= 0;
    }
    ...
    public BigFraction sqrt() {
        if (!gteq(numerator_, BigInteger.ZERO))
            return null;
	...
	if (t instanceof BigInteger) {
	    t = new BigFraction((BigInteger)t, BigInteger.ONE);
	}
	if (t instanceof BigFraction) {
	    BigFraction b = (BigFraction)t;
	    BigInteger an = numerator();
	    BigInteger ad = denominator();
	    BigInteger bn = b.numerator();
	    BigInteger bd = b.denominator();
    ...
    public boolean equals(Object other) {
    	return (other != null && other instanceof BigFraction && compareTo(other) == 0);
    }
    ...
    @Override
    public int hashCode() {
        int num = numerator().hashCode();
        int den = denominator().hashCode();
        return num * 97 + den * 13;
    }
}
